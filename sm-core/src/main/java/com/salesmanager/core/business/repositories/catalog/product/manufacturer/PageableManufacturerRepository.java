...
public interface PageableManufacturerRepository extends PagingAndSortingRepository<Manufacturer, Long> {
  ...
  @Query("select m from Manufacturer m left join m.descriptions md inner join m.merchantStore ms where ms.id=?1 and md.language.id=?2")
  Page<Manufacturer> findByStore(Integer storeId, Integer languageId, Pageable pageable);  
